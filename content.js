...
function getNodesMatching(tagName, innerTextPattern, className) {
  ...
  for (var i in nodes) {
    if (nodes[i].innerText && nodes[i].innerText.match(innerTextPattern)) {
      console.log(
        "unsubscribe-button:pattern matched node: " +
          tagName +
          " : " +
          innerTextPattern.toString() +
          " : " +
          nodes[i].innerText
      );
...
function chooseBestUnsubLink(unsubLinks) {
  for (var i = unsubLinks.length - 1; i >= 0; i--) {
    // start at end, as unsub link typically at bottom
...
function openBestUnsubLinkInEmailBody() {
  ...
  if (link) {
    ...
    displayModalForTime(
      'Couldn\'t find unsubscribe link.<br />Use the "!" shortcut to report spam.<br /><br />¯\\_(ツ)_/¯',
      5000
    );
  }
}
...
function registerUnsubscribeKeyboardShortcutListener(reregister) {
  ...
  document.addEventListener(
    "keyup",
    unsubscribeKeyboardShortcutListener,
    false
  );
...
function incrementUnsubCount() {
  getUnsubCount(function (unsubCount) {
    ...
    chrome.storage.sync.set(data, function () {
      console.log("unsubscribe-button:sync.set", data);
    });
  });
}
...
function addUnsubShareNode() {
  getUnsubCount(function (unsubCount) {
    ...
    plusNode.parentNode.parentNode.parentNode.insertBefore(
      unsubShareNode,
      plusNode.parentNode.parentNode
    );
    ...
    UnsubscribeButton.timeoutRef = setTimeout(
      removeUnsubShareNode,
      removeUnsubShareNodeAfterSeconds
    );
  });
}
...
function unsubShareNodeHtml(unsubCount) {
  ...
  fburl +=
    "&name=" +
    encodeURIComponent(
      "Unsubscribed from " + unsubCount + " email newsletters"
    );
  fburl +=
    "&caption=" + encodeURIComponent("Using Unsubscribe Button for Chrome");
  fburl +=
    "&link=" +
    encodeURIComponent(
  ...
  fburl +=
    "&redirect_uri=" +
    encodeURIComponent("http://kortina.net/work/thanks-for-sharing");
